...
class BroadbandchoicesDongleParser(BaseParser):
    ...
    URL = "https://www.broadbandchoices.co.uk/mobile/results/devicescontracts?isDataOnlyDevice=true&page={}&deviceCondition=New&unlimitedData=true&unlimitedTexts=false&unlimitedMinutes=false&includeResellers=true&includeExistingCustomersHandset=false"
    ...
    def parse(self) -> List[WiFiDongle]:
        ...
        while len(deals) > 0:
            ...
            for deal in tqdm.tqdm(deals):
                ...
                upfront_cost = (
                    deal.get("tariff", {}).get("upfrontCosts", {}).get("sortValue")
                )
                ...
                contract_months = (
                    deal.get("tariff", {}).get("contractLength", {}).get("sortValue")
                )
